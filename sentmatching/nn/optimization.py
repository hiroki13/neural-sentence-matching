...
def create_optimization_updates(cost, params, method="sgd", max_norm=5, updates=None, gradients=None,
                                lr=0.01, eps=None, rho=0.99, gamma=0.999, beta1=0.9, beta2=0.999, momentum=0.0):
    ...
    gsums = create_accumulators(params) if method != "sgd" or _momentum > 0.0 else [None for p in params]
...
def is_subtensor_op(p):
    if hasattr(p, 'owner') and hasattr(p.owner, 'op'):
        return isinstance(p.owner.op, T.AdvancedSubtensor1) or isinstance(p.owner.op, T.Subtensor)
    return False
...
def create_accumulators(params):
    ...
    for p in params:
        if is_subtensor_op(p):
            ...
            acc = theano.shared(np.zeros_like(origin.get_value(borrow=True), dtype=theano.config.floatX))
        else:
            acc = theano.shared(np.zeros_like(p.get_value(borrow=True), dtype=theano.config.floatX))
        accums.append(acc)
    return accums
...
def create_adagrad_updates(updates, params, gparams, gsums, lr, eps):
    for p, g, acc in zip(params, gparams, gsums):
        if is_subtensor_op(p):
            ...
            updates[acc] = T.set_subtensor(acc_slices, new_acc)
            updates[origin] = T.inc_subtensor(p, - lr * (g / T.sqrt(new_acc + eps)))
        else:
            new_acc = acc + g**2
            updates[acc] = new_acc
...
def create_adadelta_updates(updates, params, gparams, gsums, xsums, lr, eps, rho):
...
def create_esgd_updates(updates, params, gparams, gsums, xsums, lr, eps, gamma, momentum):
    has_momentum = momentum.get_value() > 0.0
    samples = [default_mrng.normal(size=p.shape, avg=0, std=1, dtype=theano.config.floatX) for p in params]
